...
| `skills/` | 自定义 skill | ⚠️ 仅自定义 | 插件自带的 skill 不放,靠插件清单 |
| `hooks/` | hook 脚本 | ⚠️ 谨慎 | shell/python 脚本要跨平台适配 |
| `plugins/installed_plugins.json` | 插件清单 | ✅ 同步 | 目标机器据此装同样插件 |
...
**Q:skills 文件夹里有些是插件装的,要不要同步?**
A:不用。插件 skill 跟着 `plugins/installed_plugins.json` 走 —— 在目标机器装上同名插件就有了。仓库里只放**你手写的** skill。

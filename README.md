...
> 🆕 **不想敲命令行?** 直接双击仓库根目录的 [`sync.html`](sync.html) —— 一个图形化配置合并向导(单文件、零 Web Server、离线可用):勾选本机要同步的数据 → 导入另一台机器的备份 → 图形化比对差异 → 一键生成脱敏基线 zip + `install.sh`。下方命令行方式(`pack-claude-config.sh` + 手动 diff)依然保留,适合无 GUI / SSH / CI 场景。
...
├── README.md                  # 你正在看的这份
├── LICENSE                    # MIT
├── sync.html                  # 🆕 图形化配置合并向导(双击即用,零 Web Server)
├── scripts/
